...
export default function ProjectSection({ id, lang, title, titleAr, location, locationAr, descEn, descAr, bullets, bulletsAr, images, darkBanner, bgLight }: ProjectSectionProps) {
  const isAr = lang === "ar"
  ...
  const bg = bgLight ? "bg-muted/20" : "bg-white"

  return (
    <section id={id} className={`${bg}`} dir={isAr ? "rtl" : "ltr"}>
      {/* Dark banner with project name */}
      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-8 py-14">
        <div className="grid lg:grid-cols-2 gap-12 items-start mb-12">
          {/* Left: Info */}
          <div>
            <p className="text-xs font-bold tracking-widest uppercase text-primary mb-3">{isAr ? `عن ${titleAr}` : `ABOUT ${title}`}</p>
            <h3 className="text-2xl font-black text-foreground mb-1">{isAr ? titleAr : title}</h3>
            <p className="text-xs text-muted-foreground tracking-wide mb-4">{isAr ? locationAr : location}</p>
            <p className="text-sm text-muted-foreground leading-relaxed mb-5">{isAr ? descAr : descEn}</p>
        ...
        <div className="relative mb-12">
          <div className="relative aspect-[16/7] overflow-hidden">
            ...
            <div className="absolute bottom-6 inset-x-0 text-center">
              <p className="text-white font-black text-xl tracking-widest uppercase">{isAr ? titleAr : title}</p>
            </div>
            {/* Nav arrows */}
            <button onClick={() => setImgIdx((imgIdx - 1 + images.length) % images.length)}
              className="absolute left-4 top-1/2 -translate-y-1/2 bg-black/40 text-white p-2 hover:bg-black/60 transition-colors">
              <ChevronLeft className="w-5 h-5" />
            </button>
            <button onClick={() => setImgIdx((imgIdx + 1) % images.length)}
              className="absolute right-4 top-1/2 -translate-y-1/2 bg-black/40 text-white p-2 hover:bg-black/60 transition-colors">
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-start">
          <div>
            ...
            <p className="text-sm text-muted-foreground mb-4">
              {isAr ? "شارك معلوماتك اليوم وسيتواصل معك فريق المبيعات لدينا على الفور لتزويدك بمعلومات شخصية والإجابة على جميع أسئلتك. فريقنا متاح ٢٤ ساعة يومياً، ٧ أيام أسبوعياً."
            ...
            <h4 className="text-base font-bold text-foreground mb-4">{isAr ? "تواصل للمزيد من المعلومات" : "Get in touch for more information"}</h4>
            <div className="flex gap-3">
              <a href="https://wa.me/201110944499" target="_blank" rel="noopener noreferrer"
                className="w-10 h-10 bg-green-500 text-white flex items-center justify-center hover:bg-green-600 transition-colors">
          ...
          <div className="space-y-3" dir={isAr ? "rtl" : "ltr"}>
            <form onSubmit={submit} className="space-y-3">
              <Input placeholder={isAr ? "الاسم الكريم" : "Full Name"} value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required className="h-11 border-border" />
              <Input type="email" placeholder={isAr ? "البريد الإلكتروني" : "Email"} value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} className="h-11 border-border" />
              <Input type="tel" placeholder={isAr ? "رقم الهاتف" : "Mobile"} value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} required className="h-11 border-border" dir="ltr" />
